...
as.factor(sce$Sample) ->sce$Sample
multiplot(
  plotColData(
    object = sce, 
    y = "sum",
...
```{r}
tabyl(data.frame(label.fine = pred_cell_fine$labels), label.fine) %>%
  adorn_pct_formatting(digits = 1) %>% 
  dplyr::arrange(desc(n)) %>%
  knitr::kable(
    caption = "Cell label assignments using the fine labels of the reference data sets.")
```
...
tab <- table(Assigned = pred_cell_fine$pruned.labels, Cluster = sce$cluster)
```
```{r}
library(pheatmap)
pdf("seurat_pheatmap_cluster_labels_fine.pdf",height = 7,width=8)
